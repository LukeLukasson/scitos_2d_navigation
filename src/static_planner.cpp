...
void StaticPlanner::goal_cb(const geometry_msgs::PoseStampedConstPtr &msg)
{
    ...
    ROS_WARN("+++ Callback new goal");
        ...
        while(ros::ok() && !check_path_is_active) {
            ...
            if(!planned) {
                ROS_WARN("Could not plan to the current goal on the static_map...");
            }
...
StaticPlanner::ReobservationState StaticPlanner::check_path()
{
    ...
    ROS_INFO_STREAM("Time since call: " << ros::Time::now() - time_at_call);
        ...
        if(value_map > 90) {
            ...
            if(check_if_in_sight(crit_dyn_obs)) {
                ...
                if(pose_is_reachable(original_goal)) {
                    // pose reachable
                    ROS_INFO("Original goal reachable without moving without interaction");
                    ROS_INFO_STREAM("Time since call: " << ros::Time::now() - time_at_call);
                    ...
                    if(interact_and_check()) {
                        ...
                        // pose reachable
                        ROS_INFO("Original goal reachable without moving with interaction");
                        ROS_INFO_STREAM("Time since call: " << ros::Time::now() - time_at_call);
                        sound_play::Sound success_sound = sound_client.voiceSound("Thanks for helping me.");
                        success_sound.play();
                        state = ReobservationState::succeeded;          
                ...
                if(run_mobile_reobservation(observation_polygon)) {
                    ...
                    if(pose_is_reachable(original_goal)) {
                        ...
                        // pose reachable
                        ROS_INFO("Original goal reachable with moving without interaction");
                        ROS_INFO_STREAM("Time since call: " << ros::Time::now() - time_at_call);
                        ...
                        if(interact_and_check()) {
                            // dynamic obstacle moved -> Go to original target
                            // recovery successful
                            ROS_INFO("Original goal reachable with moving with interaction");
                            ROS_INFO_STREAM("Time since call: " << ros::Time::now() - time_at_call);
                            sound_play::Sound success_sound = sound_client.voiceSound("Thanks for helping me.");
                            success_sound.play();
                            state = ReobservationState::succeeded;
...
bool StaticPlanner::send_rosie_to_pose(const geometry_msgs::PoseStamped &pose)
{
    ...
    move_base_client->cancelAllGoals();
    // ToDo: add preempt check
    
    double x = pose.pose.position.x;
    double y = pose.pose.position.y;    
    ROS_INFO_STREAM("  Send Rosie to (x,y): (" << x << "," << y << ")");
    ROS_INFO_STREAM("Time since call: " << ros::Time::now() - time_at_call);
    //move_base_client->sendGoalAndWait(send_rosie_action.action_goal.goal, ros::Duration(20.0), ros::Duration(10.0));
    move_base_client->sendGoal(send_rosie_action.action_goal.goal);
    move_base_client->waitForResult();//ros::Duration(20.0)
    ROS_INFO_STREAM("  After calling move_base: (" << x << "," << y << ")");
    ROS_INFO_STREAM("Time since call: " << ros::Time::now() - time_at_call);
    if(move_base_client->getState() == actionlib::SimpleClientGoalState::SUCCEEDED) {
...
bool StaticPlanner::run_mobile_reobservation(const perceive_tabletop_action::FindGoalPoseGoal &observation_polygon)
{
    ...
    int counter_obervations = 0;
    int counter_reach_obervation = 0;
    ...
    while(!reached_observation_pose && counter_reach_obervation < 3) {
        
        // inner loop: 10s to find a valid observation pose
        counter_obervations = 0;
        while(!observation_pose_reachable && counter_obervations < 5) {
            
            // send polygon to pose evaluation client
            pose_client->sendGoal(observation_polygon);
            bool finished_before_timeout = pose_client->waitForResult();
        ...
        observation_pose_reachable = false;
        counter_reach_obervation++;
    }
...
void StaticPlanner::execute_cb(const scitos_2d_navigation::UnleashStaticPlannerGoalConstPtr &goal)
{
	...
	ROS_ERROR("Finished action server call");
}
