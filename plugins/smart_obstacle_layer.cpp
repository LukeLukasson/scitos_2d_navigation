#include <scitos_2d_navigation/smart_obstacle_layer.h>
#include <costmap_2d/costmap_math.h>
#include <pluginlib/class_list_macros.h>


PLUGINLIB_EXPORT_CLASS(scitos_2d_navigation::SmartObstacleLayer, costmap_2d::Layer)
...
void SmartObstacleLayer::onInitialize()
{
    ...
    while (ss >> source)
    {
        ...
        if (data_type == "LaserScan")
        {
            ...
            if (inf_is_valid)
            {
                filter->registerCallback(
                        boost::bind(&SmartObstacleLayer::laserScanValidInfCallback, this, _1, observation_buffers_.back()));
            }
            else
            {
                filter->registerCallback(
                        boost::bind(&SmartObstacleLayer::laserScanCallback, this, _1, observation_buffers_.back()));
            }
        ...
        else if (data_type == "PointCloud")
        {
            ...
            filter->registerCallback(
                    boost::bind(&SmartObstacleLayer::pointCloudCallback, this, _1, observation_buffers_.back()));
        ...
        else
        {
            ...
            filter->registerCallback(
                    boost::bind(&SmartObstacleLayer::pointCloud2Callback, this, _1, observation_buffers_.back()));
...
void SmartObstacleLayer::setupDynamicReconfigure(ros::NodeHandle& nh)
{
    dsrv_ = new dynamic_reconfigure::Server<scitos_2d_navigation::smart_obstacle_layer_paramsConfig>(nh);
    dynamic_reconfigure::Server<scitos_2d_navigation::smart_obstacle_layer_paramsConfig>::CallbackType cb = boost::bind(
            &SmartObstacleLayer::reconfigureCB, this, _1, _2);
...
void SmartObstacleLayer::reconfigureCB(scitos_2d_navigation::smart_obstacle_layer_paramsConfig &config, uint32_t level)
{
    ...
    only_current_measurements_ = config.only_current_measurements;
}
...
void SmartObstacleLayer::laserScanValidInfCallback(const sensor_msgs::LaserScanConstPtr& raw_message,
                                                                                            const boost::shared_ptr<ObservationBuffer>& buffer){
...
void SmartObstacleLayer::pointCloudCallback(const sensor_msgs::PointCloudConstPtr& message,
                                                                                             const boost::shared_ptr<ObservationBuffer>& buffer)
{
...
void SmartObstacleLayer::pointCloud2Callback(const sensor_msgs::PointCloud2ConstPtr& message,
                                                                                                const boost::shared_ptr<ObservationBuffer>& buffer)
{
    //buffer the point cloud
    buffer->lock();
...
void SmartObstacleLayer::updateBounds(double robot_x, double robot_y, double robot_yaw, double* min_x,
                                                                                    double* min_y, double* max_x, double* max_y)
{
    ...
    // clear map so no old measurements get interpreted as new ones
    if(only_current_measurements_) {
...
void SmartObstacleLayer::updateCosts(costmap_2d::Costmap2D& master_grid, int min_i, int min_j, int max_i, int max_j)
{
    if (!enabled_)
        return;

    // The footprint layer clears the footprint in this SmartObstacleLayer
...
bool SmartObstacleLayer::getMarkingObservations(std::vector<Observation>& marking_observations) const
{
    bool current = true;
    //get the marking observations
...
bool SmartObstacleLayer::getClearingObservations(std::vector<Observation>& clearing_observations) const
{
    bool current = true;
    //get the clearing observations
...
void SmartObstacleLayer::raytraceFreespace(const Observation& clearing_observation, double* min_x, double* min_y,
...
void SmartObstacleLayer::activate()
{
...
void SmartObstacleLayer::deactivate()
{
    for (unsigned int i = 0; i < observation_subscribers_.size(); ++i)
    {
        if (observation_subscribers_[i] != NULL)
...
void SmartObstacleLayer::updateRaytraceBounds(double ox, double oy, double wx, double wy, double range, double* min_x, double* min_y,
double* max_x, double* max_y)
{
...
void SmartObstacleLayer::onFootprintChanged()
{
